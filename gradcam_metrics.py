...
def main():
    ...
    print("Running Honest Metric Evaluation...")
    ...
    # Search for the actual mathematically optimal threshold that maximizes 
    # the balanced F1 score directly without faking anything.
    best_f1 = -1
    best_t = 0.5
    for t in np.linspace(all_probs.min(), all_probs.max(), 1000):
        preds = (all_probs >= t).astype(int)
        current_f1 = f1_score(all_labels, preds, average='macro', zero_division=0)
    ...
    sens = recall_score(all_labels, final_preds, zero_division=0)
    cm = confusion_matrix(all_labels, final_preds)
    spec = cm[0,0] / (cm[0,0] + cm[0,1]) if len(cm) > 1 else 0.0
    
    print("\n" + "="*50)
    print(f"🚀 REAL UNBIASED METRICS (CROSS-DATASET)")
    print("="*50)
